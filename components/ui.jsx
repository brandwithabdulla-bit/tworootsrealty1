...
export function PropertyCard({item,kind='properties'}){return <article className="property-card"><Photo className="card-backdrop" src={item.images[0]} alt={`Illustrative ${item.propertyType.toLowerCase()} photography`} sizes="(max-width: 700px) 95vw, (max-width: 1100px) 50vw, 33vw"/><div className="card-top"><span className="badge">{item.status}</span><Link className="card-open" href={`/${kind}/${item.slug}`} aria-label={`View ${item.title}`}><ArrowUpRight size={16}/></Link></div><div className="card-copy"><div className="card-meta"><span>{item.propertyType}</span><span>Demo collection</span></div><h3><Link href={`/${kind}/${item.slug}`}>{item.title}</Link></h3><p className="card-location">{item.location} <span>· {item.developer}</span></p><div className="card-facts"><span>{item.bedrooms===0?'Studio':`${item.bedrooms} beds`}</span><span>{item.bathrooms} baths</span><span>{item.area.toLocaleString()} sq ft</span></div><div className="card-bottom"><p className="price">{item.priceLabel}</p><Link href={`/${kind}/${item.slug}`} className="card-details" aria-label={`Explore ${item.title}`}><span>Explore</span> <ArrowUpRight size={13}/></Link></div></div></article>}
